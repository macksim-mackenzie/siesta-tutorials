...
C translate cell
     ...
     .        xat(i,ix)=xat(i,ix)-cell(ix,ix)
      ...
      if (nspin .eq. 1) then
        ...
        fnameout(1) = paste(fnamein,'.DIFF.cube')
        fnameout(2) = paste(fnamein,'.SUM.cube')
      else 
...
C      open( unit=2, file=fnameout(isp), status='new', form='formatted')
     ...
     .    ii,(cell(ix,iy)/ii,iy=1,3)
      ...
      do iy=1,mesh(2),nskip
         if (isp.eq.1) then
        write(2,'(6e13.5)') 
     .  (rhot(ix+(iy-1)*mesh(1)+(iz-1)*mesh(1)*mesh(2),1)-
     .  rhot(ix+(iy-1)*mesh(1)+(iz-1)*mesh(1)*mesh(2),2), 
     .   iz=1,mesh(3),nskip)
      else
      write(2,'(6e13.5)')
     .  (rhot(ix+(iy-1)*mesh(1)+(iz-1)*mesh(1)*mesh(2),1)+
